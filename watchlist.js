...
async function getmovieList2(imb){
    const response =  await  fetch(`http://www.omdbapi.com/?apikey=b483166d&i=${imb}`)
    const info = await response.json()
    return fromlocal(info)
        }
...
async function render(){
    ...
    for(let list of infolist){
        temp+=await getmovieList2(list)
    }
    rener(temp)
    
    var removeEl = document.querySelector('.movie-list')
    removeEl.addEventListener('click',function(e){
        if(e.target.id){
            document.getElementById(e.target.id).innerText="✔️Removed";
            array= array.filter(function(imbs){
            return imbs!=e.target.id
            })
    ...
    <div class = "movie-main">
            ...
            <div class = "movie-info">
                ...
                <div class="time-genre-btn">
                    ...
                    <button class="btn remove" id ="${data.imdbID}">Remove</button>
                </div>
